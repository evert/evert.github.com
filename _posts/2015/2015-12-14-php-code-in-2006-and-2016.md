...
I've been doing PHP code for quite a while, and next year will be the 10 year
anniversary of this blog, which always mostly has been a PHP blog.

It occurred to me that it would be interesting to take a quick look how my PHP
code has changed in this timespan.

To do this, I dug up some old code and created two fictional code samples,
representing a class in 2006 and 2016.

The first example I did based off an old open source project that originally
had PHP 5.0.4 as the minimum version requirement. The second is how I'll start
doing things in PHP 7.
 ...
 * @licence http://sabre.io/license/ Modified BSD License 
 */
class Player {
...
* `@version $Id$` is long gone, and is something used by Subversion to
...
* Before, I would list all properties in a class first, and functions second.
  ...
  read about protected or private properties, so it will move the things that
...
* Since many years none of my sources have `?>`, which probably helped me
  avoid a bunch of bugs before I used PHPUnit everywhere.

And yea, I still don't care about [PSR-2][10]. I helped split [PSR-1][9] and 2
to ensure that people can follow the important stuff (PSR-1) while keeping
creative freedom with their sources (PSR-2).
...
* Really went all-in with [PHPUnit][7], running all my tests on [Travis][8].
...
meant it was hard to change a habit (for example mercurial. Still sad about
that). Aside from that, most of these changes were gradual, and it's kind
of interesting to take a step back and look at them all at once.
...
I think a lot of us copied a lot of things that were happening in the Java-
world, but the reality is that that's a widely inappropriate place to look
...
typing, less objects for every bit of data, less configuration and more
functional, event-based. I think many people have seen this and moved from
heavy frameworks (such as ZF1) to microframeworks (such as Silex and Slim).

Ofcourse every new year brings a new schools of thought, and the new one
seems to bring CQRS, Event Sourcing, etc, which is an influencer of the
controversial Immutable objects in PSR-7, but frankly I think that trend
is short lived and inappropriate for the PHP world. 
...
* There was no `public`, `private` and `protected`. Everything was public and
  ...
  keywords, that that was also the prime reason people put `public` for every
  function.
...
* Private members were generally prefixed with an underscore. This habit stuck
  around in many projects for years in the PHP 5 days. Personally I never
  bothered.
* CVS for the win!
