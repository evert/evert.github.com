...
or even just call `echo()` or `readfile()`.
...
I'm very much on the fence. sabre/dav is an application that heavily makes use
of all things HTTP, and is the type of application that would run into these
edge cases.
...
adapter, for various integrations and it would be nice to not need that
at one point.
