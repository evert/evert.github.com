...
that it follows a strict pattern, but they're also valid C source files.
The advantage that they can easily be statically compiled into C files without
requiring a parser.

This is really similar to the history of the JSON format, which was designed
to be a subset of Javascript, which allowed Javascript programs to read the
format without a new parser (using `eval` or similar techniques).


A deep dive into the source
---------------------------

All Neko's states are expressed using this object:
...
Then with the images and the state machine, this class does the rest:
...
for it to automatically show up here.
